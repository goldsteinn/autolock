...
#define ensure_open(...)                                               \
    CAT(ensure_open, PP_NARG(__VA_ARGS__))(__VA_ARGS__)

#define safe_read(fd, buf, count) I_safe_read(fd, buf, count, ERR_ARGS)
#define safe_write(fd, buf, count)                                     \
    I_safe_write(fd, buf, count, ERR_ARGS)

#define ensure_read(fd, buf, count)                                    \
    I_ensure_read(fd, buf, count, ERR_ARGS)
#define ensure_write(fd, buf, count)                                   \
    I_ensure_write(fd, buf, count, ERR_ARGS)
...
#define safe_open3(path, flags, mode)                                  \
    I_safe_open3(path, flags, mode, ERR_ARGS)
