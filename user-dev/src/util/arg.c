...
static const char * type2fmt[] = { "",         "<int>",  "<char>",
                                   "<string>", "<bool>", "<float>",
                                   "<rest>",   "",       "",
                                   "" };
...
static void usage(char const * restrict pname,
                  ArgParser const * restrict ap);

static void
FORMATF(2, 3) argdie(ArgParser const * restrict def,
...
concat_args(char const * const * args_begin) {
    ...
    for (; args_begin && *args_begin; ++args_begin) {
        ...
        next =
            stpncpy_c(buf + offset, *args_begin, OUTBUF_LEN - offset);
...
arg2str(ArgOption const * restrict desc) {
    ...
    switch (desc->type) {
        ...
        case Float:
            n = snprintf(
                p, avail_length, "%s%c%s",
                concat_args(desc->args_begin), sep,
                type2fmt
                    [desc->type] /* NOLINT(*-constant-array-index) */);
...
usage(char const * restrict pname, ArgParser const * restrict ap) {
    ...
    for (apn = ap->parsers; apn; apn = apn->next) {
        ...
        int32_t           i    = 0;
        while (args[i].kind != KindEnd) {
            fprintf(stderr, "   %20s\t%s\t", arg2str(args + i),
                    args[i].kind == KindHelp ? "Print this message"
                                             : args[i].desc);
            switch (args[i].type) {
                case Increment:
                case Integer: {
...
#define I_CHECK_OVERFLOW_ERROR(unsigned_T, signed_T)                   \
    if ((is_negative || (!desc->is_unsigned)) &&                       \
        (CAST(int64_t, CAST(signed_T, user_v)) !=                      \
         CAST(int64_t, user_v))) {                                     \
        storage_type = desc->is_unsigned ? V_TO_STR(unsigned_T)        \
                                         : V_TO_STR(signed_T);         \
        err          = OVERFLOW;                                       \
    }                                                                  \
...
assignArg(ArgOption * restrict desc,
    ...
    switch (desc->type) {
        ...
        case Float: {
            ...
            warn_assert(argv[offset] != end,
                        "Float -> [%s] unparseable (%s)\n",
        ...
        case Integer: {
            ...
            if (argv[offset] != end) {
                ...
                warn_assert(
                    err == 0,
                    "int -> [%s] overflow converting (%s) to %s\n",
                    concat_args(desc->args_begin), argv[offset],
                    storage_type);
            }
            return 1;
...
checkArgDef(ArgParser const * restrict ap,
    ...
    for (i = 0; desc[i].kind != KindEnd; i++) {
        ...
        if (((state == KindPositional) || (state == KindRest)) &&
            !main) {
...
createArgumentParser(ArgDefs const * restrict def) {
    ...
    ap->parsers =
        (ArgParserNode *)safe_calloc(1, sizeof(ArgParserNode));
...
int32_t
parseArguments(ArgParser * restrict ap,
               int32_t        argc,
               char * const * argv) {
    ...
    for (i = 0; (i < argc) && optionsPossible; i++) {
        ...
        if (arg[0] == '-') {
            ...
            for (apn = ap->parsers; notfound && apn; apn = apn->next) {
                ArgOption * desc = apn->parser->args;
                for (j = 0; notfound && (desc[j].kind != KindEnd);
                     j++) {
                    if (user_args_match(desc[j].args_begin, arg)) {
                        ...
                        /* process it.  */
                        int32_t consumed = assignArg(desc + j, argv + i,
                                                     ap, 1, argv_end);
                        if (consumed < 0) {
                            argdie(
                                ap,
    ...
    while ((desc[baseDestOffset + j].kind == KindPositional) &&
        ...
        int32_t consumed =
            assignArg(desc + baseDestOffset + j, argv + baseArg + j, ap,
                      0, argv_end);
        j += consumed;
    ...
    if (desc[baseDestOffset + j].type == Rest &&
        ((baseArg + j) < argc)) {
        int32_t consumed =
            assignArg(desc + baseDestOffset + j, argv + baseArg + j, ap,
                      0, argv_end);
