...
parser = argparse.ArgumentParser(
    description='process output of micro benchmark')
...
parser.add_argument("-o",
                    "--targetdir",
                    default=".",
                    help="where to put all gnuplot files")
parser.add_argument("-w",
                    "--warnIsError",
                    action="store_true",
                    help="don't die on warnings")
parser.add_argument("-b",
                    "--skipBadUsage",
                    action="store_true",
                    help="if start usage is too high, skip data")
parser.add_argument("-a",
                    "--againstBest",
                    action="store_true",
                    help="plot all against best")
...
known = {
    ...
    'block': {
        'name': 'lock',
        'column': 'lock',
        'help': 'name of lock second time in file'
    },
    'cpu_mask': {
        'name': 'cpu_mask',
        'column': 'cpu_mask',
        'help': 'where threads are run'
    },
    'is_pinned': {
        'name': 'is_pinned',
        'column': 'is_pinned',
        'help': 'whether threads are pinned or not'
    },
    'cs_iter': {
        'name': 'cs_iter',
        'column': 'cs_iter',
    ...
    'cpu_usage_start': {
        'name': 'cpu_usage_start',
        'column': 'cpu_usage_start',
        'chkerr': True,
        'help': 'cpu_usage_start'
    },
    'cpu_usage_end': {
        'name': 'cpu_usage_end',
        'column': 'cpu_usage_end',
        'chkerr': True,
        'help': 'cpu_usage_end'
    },
    'min': {
...
class Run():
    ...
    def addrun(self, numProcs, cs_iter, extra_iter, outer_iter, cstart, cend,
               mincnt, maxcnt, geo):
        ...
        if key in self.runs[numProcs]:
            wprint(
                0, 0, "{} {} on {} procs with {} already exists".format(
                    self.base, self.kind, numProcs, key))
        # add cnt scaled to number of outer_iter's
        self.runs[numProcs][key] = {
            'cstart': cstart,
            'cend': cend,
            'min': mincnt / outer_iter,
            'max': maxcnt / outer_iter,
            'geo': geo / outer_iter
        }
...
def processFile(inname, infile):
    ...
    for name in headers:
        if name not in known:
            wprint(0,
                   0,
                   "{} is an unknown column from {}".format(name, inname),
                   die=False)
            warning += 1
        else:
    ...
    for name in known.keys():
        if known[name].get('checked', 0) != 1:
            wprint(0,
                   0,
                   "{} is not in csv file: {}".format(name, inname),
                   die=False)
            warning += 1
    ...
    for line in reader:
            ...
            if known[fld].get('chkerr', False):
                if line[fld] == 'ERROR':
                    wprint(
                        inname,
                        ln,
                        "skipping {} {} because has ERROR in some needed columns",
                        die=False)
                    hasErrors = True
                    break
        ...
        locks[base][kind].addrun(int(line['nthreads']), int(line['cs_iter']),
                                 int(line['extra_iter']),
                                 int(line['outer_iter']),
                                 float(line['cpu_usage_start']),
                                 float(line['cpu_usage_end']),
                                 float(line['min']), float(line['max']),
                                 float(line['geomean']))

        if line['is_pinned'] != '1':
            wprint(
                inname, ln, "threads in run are not pinned: '{}'".format(
                    line['is_pinned']))
        ...
        for (nproc, iters) in runlist:
            ...
            for oneiter in iters:
                ...
                if autorun is None:
                    wprint(
                        0, 0,
                        "normal lock present, autolock absent for {} {} {}".
                        format(lock, nproc, oneiter))
                bound = max(1, autorun['cstart'] * .3)
                if abs(autorun['cstart'] -
                       norm.getCstart(nproc, oneiter)) > bound:
                    if skipBadUsage:
                        ...
                        wprint(
                            0,
                            0,
                            "cpu usage at start is very differet for {} {} {}: Normal:{}, Auto:{}"
                            .format(lock, nproc, oneiter, autorun['cstart'],
        ...
        for oneiter in sorted(alliters.keys()):
            ...
            for nproc in everyproc:
                ...
                if normdata is None or autodata is None:
                    ...
                    outf.write("{}, {}, {}, {}\n".format(
                        nproc, normdata['geo'] / autodata['geo'],
                        normdata['min'] / autodata['min'],
                        normdata['max'] / autodata['max']))
            outf.close()
            outdata.append(line)
...
def createGPbyProc(outpath):
    ...
    for nproc in everyproc:
        ...
        for oneiter in everyiter:
            ...
            plotline += ", '{}' using 1:2:4:xticlabel(3) with boxes linecolor variable title '{}'".format(
                name, oneiter)
            outf = open(os.path.join(outpath, name), "w")
            bestdata = denoms[oneiter].getRunData(nproc, oneiter)
            bestlock = "{} {}".format(denoms[oneiter].base,
                                      denoms[oneiter].kind[0:4])
            gp.write("set label at first {},{} front \"{} {}\"\n".format(
                xoffset, maxy - (maxy - miny) * .2, oneiter, bestlock))
            for lock in sorted(locks.keys()):
                for kind in ['normlock', 'autolock']:
                    thisdata = locks[lock][kind].getRunData(nproc, oneiter)
                    if thisdata is None:
                        outf.write("{}, {}, {}-{}, {}\n".format(
                            xoffset, '-', lock, kind, colors[kind]))
                    else:
                        outf.write("{}, {}, {} {}\n".format(
                            xoffset, thisdata['geo'] / bestdata['geo'], lock,
                            colors[kind]["num"]))
                    xoffset += 1
            outf.close()
            xoffset += 1
